...
- **Surface ownership** — `ui::EditorSurface::render` owns the DirectWrite
  factory/text format, device-dependent renderer, active `FontStateId`, bounded
  text-layout cache, shared row-count caches, and focused-table layout cache.
  The owning UI thread is the sole renderer/layout mutator. `ui::Window` keeps
  only host geometry and lifecycle inputs such as HWND client pixels, live DPI,
  and a deferred live-resize target, then drives the surface resources through
  the existing paint and resize handlers.
...
- **Effective line height** — the row stride is derived, not constant: `Window::effective_line_height() = round(scaled_font_size × [editor].line_height)`, where `scaled_font_size = base font size × per-pane zoom (`ViewState.font_size_scale`)`. Ctrl+wheel zoom therefore scales row height in lock-step with glyph size (rows can't overlap). This one value is the canonical *vertical* geometry for the paint stride (`DrawParams::line_height`) **and** every UI-side geometry consumer — scroll math, hit-test, caret anchor, content-height, image-row reservations. The `LINE_HEIGHT_DIP = 20.0` constant survives only as the zoom-1 / multiplier-1 reference value and the fixed one-line EOF bottom inset (`END_OF_BUFFER_BOTTOM_PADDING_DIP`).
...
- **Caret visibility after edits** — `Window::ensure_primary_caret_visible`
  ...
  `EditorSurface::pending_doc_end_scroll` after moving the caret, then `on_paint`
...
- **Fixed-row heading guard**: markdown headings use the restrained `[1.20, 1.16, 1.12, 1.08, 1.04, 1.02]` scale. `row_text_paint::draw_text_layout_in_fixed_row` clips focused no-wrap, focused soft-wrap, and spectator-pane glyph output to the existing display-row height. The display map, row stride, scroll extent, hit testing, and caret-line screen-y remain unchanged; oversized glyphs cannot paint into the following row.
...
- **Current-line band**: `chrome::paint_current_line_highlight(..., display_rows: Option<(u32, u32)>)`. Wrap-aware via `(first_display_row, row_count)`. Gated on `view_options.current_line_highlight` (default `true`). Fills with its **own** brush built from `EditorColors::caret_line_highlight` (theme key `editor.caret_line_highlight`) — distinct from the mouse-hover band, which still derives a scaled-alpha brush from `editor.line_highlight` (see `crate::line_bands::paint_line_bands`).
- **Line-number gutter**: `chrome_line_numbers::paint_line_number_gutter(...)` takes `hovered_source_line: Option<usize>` (fed from `params.line_hover.source_line`). In `gutter_caret_line_only` mode it stamps the caret line (active brush) plus muted first/last viewport anchors **and** the hovered line's number (muted, via `add_hover_anchor_label`) so the pointer's line reads without turning on all numbers; ignored on the full-numbers path. The display-row branch (`paint_gutter_display_rows`, taken when a `FrameDisplay` is supplied) skips `is_wrap_continuation` rows so wrapped paragraphs label only their first row.
- **Fold ticks**: `chrome_fold::paint_fold_triangles(..., gutter_hovered: bool)` gates by `should_paint_fold_tick(is_folded, gutter_hovered)` — collapsed `▸` glyphs always paint (a folded section stays discoverable); expanded `▾` glyphs paint only while `params.line_hover.in_gutter`. Early-returns when the gutter is un-hovered and `folded_lines` is empty.
- **Indent guides**: `chrome_indent_guides::paint_indent_guides(..., frame_display, ..., active_caret_source_line, color, active_color)` iterates the `FrameDisplay` **display-row** grid (not the source-line grid, which misaligned under soft-wrap), paints `k in 0..depth` so the `k == 0` body-left-edge column is drawn (it is the top-level parent, and the only guide a singly-indented line has; it clears the gutter↔body divider by `GUTTER_BODY_GAP_DIP`, and there is no divider at all when line numbers are off), paints wrap-continuation rows with their source line's columns so a wrapped paragraph carries an unbroken rule, and emphasizes the caret source line's deepest guide column with `active_color` (`indent_guide_active`).
- **Status bar**: `status_bar::paint_status_bar(...)` consumes `StatusBarData`; caller passes `client_height_dip - STATUS_BAR_HEIGHT_DIP` as `top`. `compute_layout` snaps every segment / chip `left` and `right` bound to the nearest integer DIP — `slot_width_dip` is `chars * font_size * 0.55` and would otherwise accumulate fractional offsets that make ClearType re-sample changing-digit text against a different sub-pixel grid each paint (perceived as blurry counters). With rounding, same-text repaints are pixel-identical. **The bar is fixed chrome — its text does not grow with body zoom.** `paint_status_bar_frame_text` is fed the *unscaled* base size (`base_font_size_dip / view.font_size_scale`) and `draw_segment` pins each layout's glyph size with `SetFontSize`, so Ctrl+wheel zoom never clips the fixed-height (`STATUS_BAR_HEIGHT_DIP` = 22) bar. **The bar never wraps:** `draw_segment` sets `DWRITE_WORD_WRAPPING_NO_WRAP` on the layout and draws with `D2D1_DRAW_TEXT_OPTIONS_CLIP`, so an over-budget chip (the mixed-line-ending warning is the remaining corrective chip) clips at its slot instead of wrapping onto a second visual line below the 22-DIP bar. Mixed-indentation diagnostics are intentionally absent from the bar.
...
- chrome (current-line band, trailing-whitespace, ruler, whitespace markers): `crates/render/src/chrome.rs`; post-text orchestration: `chrome_post.rs`; caret rect: `chrome_caret.rs`
- gutter line numbers: `crates/render/src/chrome_line_numbers.rs`; fold ticks: `crates/render/src/chrome_fold.rs`; indent guides: `crates/render/src/chrome_indent_guides.rs`
- minimap layout + paint: `crates/render/src/minimap.rs`, `crates/render/src/minimap_paint.rs`; minimap pass: `crates/render/src/renderer_draw_main/minimap_pass.rs`
- per-line wrap-aware paint: `crates/render/src/wrap_paint.rs`
- fixed-row glyph clip: `crates/render/src/row_text_paint.rs`
