...
//! thread starts. Portable mode applies process environment overrides
//! before persistence/config owners are constructed.
...
mod tests {
    ...
    fn installed_mode_keeps_path_args() {
        let opts =
            StartupOptions::from_args([OsString::from("note.md"), OsString::from("missing-dir")])
    ...
    fn portable_flag_is_not_treated_as_file() {
        let opts =
            StartupOptions::from_args([OsString::from("--portable"), OsString::from("a.md")])
