...
- **Palette**:
  ...
  - The bare `theme` query lands on the palette-mode picker `view.pick_theme`, not the theme-management commands. Two mechanisms in `palette_rank.rs`: a synthetic `"theme"` alias on `view.pick_theme` (`aliases_for`) earns the exact/prefix `field_boost`, and `command_query_bias` applies a small score *delta* (`-150`) to secondary `theme.*` commands (`theme.{clone,duplicate,rename,delete,edit,reveal_folder,create_blank,reload}`) for that exact query. The delta is a soft curation, not a hard filter — secondary commands still match and score, and the recency tiebreaker in `Palette::refilter` can still float a command the user actually picks. There is no `view.cycle_theme` command (removed; the palette never surfaces a cycle-theme entry).
