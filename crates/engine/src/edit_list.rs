...
use crate::edit_planning::{
    finalize_specs_with_transformed_selections, line_content_end, EditSpec,
};
    ...
    for selection in &selections_before {
        ...
        match list_newline_action(&indent, body, "  ") {
            ListNewlineAction::Continue { next_marker } => {
                let inserted = format!("\n{indent}{next_marker}");
                specs.push(EditSpec::insert(rope, byte, inserted)?);
            }
            ListNewlineAction::End { replacement } => {
                specs.push(EditSpec::replace(rope, line_start, line_end, replacement)?);
            }
