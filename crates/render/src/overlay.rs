...
use windows::Win32::Graphics::DirectWrite::{
    IDWriteFactory, IDWriteTextFormat, DWRITE_HIT_TEST_METRICS, DWRITE_MEASURING_MODE_NATURAL,
};
...
pub fn paint_overlay(
    ...
    if let Some(field) = &overlay.secondary_field {
        ...
        paint_focus_field(ctx, dwrite, format, &mut brushes, field, false)?;
    }
    for row in &overlay.list_rows {
        paint_list_row(ctx, &mut brushes, format, row)?;
    }
    ...
    if let Some(footer) = &overlay.footer {
        paint_footer(ctx, &mut brushes, format, footer)?;
    }
    Ok(())
}

/// On-demand `ID2D1SolidColorBrush` factory bound to one render
/// target. `pub(crate)` so sibling paint modules
/// ([`crate::time_machine_hud_paint`]) can build brushes without
/// duplicating the `CreateSolidColorBrush` boilerplate.
pub(crate) struct BrushCache {
    render_target: ID2D1RenderTarget,
}

impl BrushCache {
    pub(crate) fn new(rt: &ID2D1RenderTarget) -> Result<Self, Error> {
        Ok(Self {
...
fn paint_focus_field(
    ...
    if !display.is_empty() {
        if !field.text.is_empty() {
            paint_focus_field_selection(dwrite, format, brushes, ctx, field, inner)?;
        }
    ...
    if is_focused {
        let caret_x = if field.text.is_empty() {
            ...
            caret_offset_in_field(dwrite, format, &field.text, field.caret_byte).unwrap_or(0.0)
        };
...
fn paint_focus_field_selection(
    ...
    let left = caret_offset_in_field(dwrite, format, &field.text, start).unwrap_or(0.0);
    let right = caret_offset_in_field(dwrite, format, &field.text, end).unwrap_or(left);
    if right <= left {
        return Ok(());
    }
    let rect = D2D_RECT_F {
...
fn paint_list_row(
    ...
    if !row.primary_text.is_empty() {
        ...
        let wide: Vec<u16> = row.primary_text.encode_utf16().collect();
        unsafe {
            ctx.DrawText(
                &wide,
                format,
    ...
    if let Some(secondary) = row.secondary_text.as_deref() {
        ...
        let wide: Vec<u16> = secondary.encode_utf16().collect();
        unsafe {
            ctx.DrawText(
                &wide,
                format,
    ...
    if let Some(kb) = row.keybinding.as_deref() {
        ...
        let wide: Vec<u16> = kb.encode_utf16().collect();
        unsafe {
            ctx.DrawText(
                &wide,
                format,
...
fn paint_footer(
    ...
    let wide: Vec<u16> = footer.text.encode_utf16().collect();
    unsafe {
        ctx.DrawText(
            &wide,
            format,
...
mod tests {
    ...
    fn rect_inset_clamps_at_zero() {
        let r = Rect::new(0.0, 0.0, 4.0, 4.0).inset(10.0, 10.0);
        assert_eq!(r.w, 0.0);
        assert_eq!(r.h, 0.0);
    }

    #[test]
    fn rect_translate_moves_origin() {
    ...
    #[test]
    fn utf8_to_utf16_basic() {
        assert_eq!(utf8_byte_to_utf16_index("abc", 0), 0);
        assert_eq!(utf8_byte_to_utf16_index("abc", 2), 2);
        // "é" is 2 utf-8 bytes, 1 utf-16 code unit.
        assert_eq!(utf8_byte_to_utf16_index("café", 5), 4);
    }

    #[test]
    fn selection_range_normalizes_to_utf8_boundaries() {
        assert_eq!(
            normalized_selection_range("a日本", Some((4, 1))),
            Some((1, 4))
        );
        assert_eq!(normalized_selection_range("abc", Some((2, 2))), None);
    }
}
