...
## Key concepts
- **`Window`** — one top-level Win32 window and desktop composition. Owns HWND, swap chain, `PaneTree`, focused-pane desktop mirrors (`buffer_id`, `language`), per-pane `panes: HashMap<PaneId, PerPaneState>`, buffer-local right-edge chrome overrides (`right_edge_chrome_by_view`), view options, theme, and spell state. Its `EditorSurface` owns the focused `ViewState`, input/composition state, and projection caches.
...
- **`PerPaneState`** — non-focused pane runtime state (buffer id, view, language, decoration revision). On focus, the view swaps through `EditorSurface::view`; buffer and language remain desktop composition mirrors on `Window`.
...
- **Two focus scopes**, both UI-thread-owned: `Window::is_window_focused` is
  the host application's foreground flag, toggled on `WM_ACTIVATEAPP`, while
  `EditorSurface::focus.has_keyboard_focus` records whether this editor control
  holds keyboard focus, toggled on `WM_SETFOCUS` / `WM_KILLFOCUS`. The split
  exists because switching between controls or two continuity windows fires
  focus messages but **not** necessarily `WM_ACTIVATEAPP`. Nested overlay text
  input focus lives beside the control flag in `EditorSurface::focus`.
- **Active-pane highlight requires focus.** `build_pane_chrome` flags a leaf
  `focused` only when it is the tree's focused pane **and**
  `Window::is_window_focused` **and**
  `EditorSurface::focus.has_keyboard_focus`. A background window (other app
  focused, or another continuity window focused) paints no active-pane border.
...
- **Tabs**:
  ...
  - Vault file-tree navigation has explicit placement: plain click replaces the focused tab, Ctrl+click inserts a permanent focused tab, and Shift+click spawns a top-level window carrying the vault root/sidebar.
  - Dropping non-image files on a pane inserts them as tabs in that pane.
  - One multi-file Explorer or startup activation creates one new top-level window and adopts every selected file as a tab in its focused pane.
- **Vault workspace chrome**: opened folder root, file-tree visibility, and drag-resized width persist in the window workspace JSON. While a vault is active, status-bar actions expose Files at bottom-left and Outline/Minimap at bottom-right; the existing per-buffer right-edge visibility model remains canonical.
...
- **Close**:
  ...
  - Closing the last tab in a pane closes the pane (D4). In a single-pane window with an open folder tree, closing the last tab instead leaves a zero-tab workspace; the next `Ctrl+W` closes the window. Without folder view, closing the last tab still closes the window. Closing the last window saves session state and quits (`AppSessionEnding`).
...
- **Single instance per data dir**: `app::single_instance` + `win::single_instance` hold a named mutex keyed by the database path. A second launch forwards its command-line paths to the running instance over a `WM_COPYDATA` message-only hub and exits. Bare launch activates the top-most visible Continuity window on the current virtual desktop; if none exists or desktop detection is unavailable, the registry spawns a fresh blank window on the invoking desktop. `--vault <root>` is forwarded as explicit vault intent: a matching window is reused only on the active virtual desktop, otherwise a fresh window opens locally. `--new-instance` bypasses. This prevents persisted-session replay without pulling the writer across desktops. See `architecture.md` § Process model and `defaults.md` § Launch + sessions.
...
**Focus switch into a pane with stale saved geometry.** A pane's `PerPaneState.view` carries the viewport size from the last time that pane was focused. A split, window resize, sidebar toggle, or splitter drag while the pane is a spectator leaves that size stale, so the next click into the pane takes the *anchored* `refresh_focused_viewport` path. The anchor holds the line the user is looking at: the caret line when it is on screen, otherwise the source line at the pane's viewport top (see `caret.md` § "Screen-y anchor across reflow"). Before that rule the anchor clamped the off-screen caret into view, so clicking into a pane whose caret sat at the document end jumped the pane to the bottom - the "clicking around split panes never stays where it was" report. Regression: `crates/ui/tests/pane_focus_keeps_scroll.rs`.
