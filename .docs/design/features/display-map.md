...
The WASM binding serializes the same block/inline spans, display segments, and
complete source/display mappings for the browser. `<continuity-editor>` renders
inactive lines from that projection and reveals canonical source on selected
lines. Browser text measurement supplies the projection width inputs; the DOM
and semantic textarea remain host presentation state. See
...
- **`SegmentHit`** — `None | Checkbox { toggle, checked } | Link { url } | FootnoteReference { label, definition } | FootnoteDefinition { label, first_reference }`. Drives click → toggle / link-follow / footnote-jump without re-parsing. Only a *replaced* (unrevealed) checkbox segment carries `SegmentHit::Checkbox`; reveal is caret-driven, so `caret_bytes_for_projection` (`window_display_prewarm/projection_inputs.rs`) only reveals a source line that has a caret on it. Clicking a rendered checkbox no longer reveals/un-renders the line: `Window::toggle_checkbox_at_byte` (`window_mouse_segment_hit.rs`) places a caret on the toggle line for the single `markdown.toggle_checkbox` dispatch, then restores the caller's prior selections so no caret lands on the line and the glyph stays rendered (one undo group; the two `set_selections` are not edits). A `Link` whose target starts with `#` is an in-document anchor: the click resolves the GitHub-style heading slug against the buffer's ATX headings and jumps the caret there instead of shelling out (`ui::window_mouse_segment_hit::jump_to_heading_anchor`).
...
- **`ProjectionStamp<FontState>`** — portable identity for every projection
  input that changes pixels, plus stable field-level mismatch classification.
  The font key is backend-defined; native uses `layout::FontStateId`. Worker
  channels and DirectWrite handles remain in `ui`.
...
- **Portable policy**: row-index lookup, dirty/splice classification, projection
  stamps, wrap/segment cache keys, and `WidthMeasure` live in `display_map`.
  Native `ui::window_projection_plan` composes those pure policies with its
  `FrameDisplay` caches and backend-specific worker scheduling.
...
### Classifier rope-advanced/no-deltas backstop
`classify_projection_build` (`crates/ui/src/window_projection_plan/classify.rs`) cold-rebuilds when `rope_advanced && rope_deltas.is_empty()`. A revision only advances on a real content edit, so "advanced + zero deltas" means no dirty/splice plan can be computed and reusing `prev`'s specs would slice out-of-bounds on paint — the historical Ctrl+Z crash (an undo path that failed to record delta history). With delta history now recorded on every revision-advancing path (see [Buffer](buffer.md) § Undo/redo record delta history) this branch is unreachable; it stays as a defense-in-depth backstop so a future path that forgets to record deltas degrades to a correct cold rebuild instead of an out-of-bounds slice.
