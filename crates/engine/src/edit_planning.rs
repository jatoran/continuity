//! Shared storage-neutral planning primitives used by every edit family.
//!
...
/// Finalize specs while computing the same cumulative selection transform
/// that [`continuity_buffer::Buffer::apply`] performs for the descending op
/// sequence. Use this for edits whose desired caret result is simply the end
/// of each insertion or replacement; explicit per-spec positions expressed
/// in the pre-edit rope do not account for earlier edits in the document.
pub(crate) fn finalize_specs_with_transformed_selections(
    mut specs: Vec<EditSpec>,
    selections_before: Vec<Selection>,
) -> Option<SelectionEditPlan> {
    if specs.is_empty() {
    ...
    specs.sort_by_key(|spec| spec.start);
    let merged = merge_specs(specs);
    let mut selections_after = selections_before.clone();
    for spec in merged.iter().rev() {
        for selection in &mut selections_after {
            selection.anchor = compute_position_after_spec(selection.anchor, spec);
            selection.head = compute_position_after_spec(selection.head, spec);
        }
    }
    let ops = merged.into_iter().rev().map(EditSpec::into_op).collect();
    Some(SelectionEditPlan {
...
fn compute_position_after_spec(position: Position, spec: &EditSpec) -> Position {
    let start = spec.start_position;
    let old_end = spec.end_position();
    let new_end = advance_position(start, &spec.inserted);
    if position < start {
        return position;
    }
    if position <= old_end {
    ...
    let lines_after_old_end = position.line.saturating_sub(old_end.line);
    let byte_in_line = if position.line == old_end.line {
        new_end
            .byte_in_line
            .saturating_add(position.byte_in_line.saturating_sub(old_end.byte_in_line))
    } else {
        position.byte_in_line
    };
    Position::new(
        new_end.line.saturating_add(lines_after_old_end),
        byte_in_line,
    )
}
