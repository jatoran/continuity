...
Native renderer/layout resources live in UI-thread-owned
`EditorSurface::render`: the DirectWrite factory and text format, D2D/DXGI
renderer, `FontStateId`, layout cache, projection-walker cache handles, and
focused-table layout cache. `Window` supplies HWND client size, DPI, and
deferred-resize lifecycle inputs; no resource crosses the UI thread during
paint or resize.
...
stamps still dedupe through `EditorSurface::projection.last_early_dispatch_stamp`.
...
| Document-end scroll | `EditorSurface::pending_doc_end_scroll` set in `crates/ui/src/window_commanding/context.rs`, applied in `crates/ui/src/window_paint/doc_end_scroll.rs` after `resolve_paint_frame_display`, with post-`EndPaint` repaint scheduling in `crates/ui/src/window_paint/epilogue.rs` |
