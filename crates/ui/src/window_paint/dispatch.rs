...
//! normal body paint and the buffer-history overlay. Call ordering of
//! `renderer.draw_buffer*` and
...
pub(crate) fn dispatch_renderer_draw(
    ...
    if history_overlay {
        // Paint regular chrome and body backdrops; the caller then
        // overlays the history panels and presents once.
