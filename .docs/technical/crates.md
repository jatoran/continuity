...
| `win` | Win32 wrappers: window class, HWND, DPI, virtual desktops, COM apartment, spell-checker | `src/window.rs`, `src/dpi.rs`, `src/virtual_desktop.rs`, `src/com.rs`, `src/spell.rs` |
...
| Crate | Responsibility | Key files |
|---|---|---|
| `core` | Singleton editor state machine; sole writer of every `Buffer` | `src/state.rs`, `src/handle.rs`, `src/dispatch.rs`, `src/message.rs`, `src/selection_edit.rs`, `src/selection_coalesce.rs`, `src/edit_inline.rs`, `src/edit_lines.rs`, `src/edit_line_text.rs`, `src/edit_words.rs`, `src/edit_list.rs`, `src/edit_markdown*.rs`, `src/edit_pairs.rs`, `src/edit_indent_shift.rs`, `src/edit_planning.rs`, `src/undo.rs`, `src/policy.rs`, `src/clock.rs` |
