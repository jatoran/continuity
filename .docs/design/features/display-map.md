...
## What it is
- Pure projection from source bytes to a visible display string per source line. Hides marker bytes, replaces list markers with `• `, collapses folds, breaks lines at soft-wrap points. Source positions stay canonical for undo/persistence/search/file-I/O; the display map is *derived* — removing it yields a degraded but correct editor.
...
- **`SegmentHit`** — `None | Checkbox { toggle, checked } | Link { url } | FootnoteReference { label, definition } | FootnoteDefinition { label, first_reference }`. Drives click → toggle / link-follow / footnote-jump without re-parsing.
...
- Wrap width is the painted body-text-column width, computed by `continuity_render::resolve_body_text_width_dip` for the focused pane from the same inputs the renderer's `ContentMargins` consume (viewport width, font size, `line_numbers`, `minimap`, `search_minimap_active`, `show_outline_sidebar`, `outline_sidebar_width_dip`, `distraction_free` + cap). Non-focused pane bodies use `continuity_render::pane_body::spectator_body_text_width_dip`, which reserves only the spectator body gutter/padding. These helpers keep each pane's wrap column lined up with its rendered right edge. The focused wrap *target* additionally reserves a small right safety gutter inside that edge — `WRAP_SAFETY_MARGIN_EM` (¼ of the zoomed font size, in `window_display_prewarm/projection_inputs.rs::display_projection_metrics`) — so glyph ink overhang / measurement rounding never poke past the visible column. Paint geometry is unchanged; wrap just fires a hair earlier.
- **Continuation rows never exceed the wrap column.** The break walker (`builder/soft_wrap.rs::grapheme_word_break_points_styled`, mirrored for row counts in `builder/row_counts.rs`) measures each continuation row's carry-over width *exactly across styled-segment boundaries* (inline code, bold, links) by tracking the running width captured at the last word break — `(running + w) − running_at_word_break` — never re-measuring a current-segment-only suffix. The old suffix re-measure under-counted cross-segment carry-overs and over-filled continuation rows past the column on multi-segment lines. `event:soft_wrap_overflow` (`technical/trace-guide.md`) is the regression detector.
