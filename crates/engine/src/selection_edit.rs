//! Selection-aware edit planning for the synchronous engine.
//!
...
use crate::edit_markdown::{
    ...
    plan_markdown_insert_image_ref, plan_markdown_insert_link, plan_markdown_toggle_checkbox,
    plan_markdown_toggle_emphasis, plan_markdown_toggle_numbered, plan_markdown_toggle_task,
...
use crate::edit_planning::{
    caret_delete_range, finalize_specs_with_transformed_selections, merge_specs,
    ranges_for_selection, EditSpec,
};
...
pub enum SelectionEdit {
    ...
    /// Toggle a `- ` bullet prefix on each covered line. Shares the
    /// content-relative planner with [`SelectionEdit::ToggleBulletAtLineStart`]
    /// (`plan_toggle_bullet_at_line_start`), so the caret stays on the same
    /// content character across add/remove and an ordered→bullet conversion.
...
pub fn plan(buffer: &Buffer, edit: &SelectionEdit) -> Result<Option<SelectionEditPlan>, Error> {
    match edit {
        ...
        SelectionEdit::MarkdownToggleBullet => plan_toggle_bullet_at_line_start(buffer),
...
fn plan_insert_text(buffer: &Buffer, text: &str) -> Result<Option<SelectionEditPlan>, Error> {
    ...
    let specs = insert_specs(buffer.rope(), &selections_before, text)?;
    Ok(finalize_specs_with_transformed_selections(
        specs,
        selections_before,
    ))
}
