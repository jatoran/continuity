...
pub(crate) use crate::find_bar_memento::FindBarMemento;
...
impl FindBar {
    ...
    ///
    /// Used on re-focus (re-pressing `editor.find` / `editor.replace` while
    /// the bar is open) so the next typed character overtypes the whole
    /// query/replace text. The anchor lands at byte 0 and the caret at the
    /// end, matching native find-bar behavior.
    pub(crate) fn select_all_focused_field(&mut self) {
        self.focused_input_mut().select_all();
    }
}
