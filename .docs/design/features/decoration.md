...
- **Pipe-table cells carry text inlines.** `inline::scan_pipe_table` emits a `TablePipe` marker per `|` *and* runs the ordinary text scanner over the block, so links, emphasis, code, strike, and image refs inside cells are spans exactly as in prose. Every display-map consumer (browser projection, native body pass, link hit-testing) then hides the markup and styles the text; the native table chrome still parses cells itself (`render::table_layout::cell_inline`) and the native inline-image pass skips image refs inside table blocks so a logo in a cell never reserves phantom rows. Before 2026-09-18 table blocks emitted only pipe markers, which left `[text](url)` raw in web cells.
