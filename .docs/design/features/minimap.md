...
- A translucent **viewport indicator** rectangle painted over the strip's section that corresponds to the visible body region. Tracks scroll continuously.
...
## Layout math
`compute_minimap_layout(pane_rect, scroll_y, line_height, total_lines, right_inset_dip)` returns:

- `rect` — strip outer rect in pane-local DIPs. `x = pane_rect.x + pane_rect.w - right_inset_dip - MINIMAP_WIDTH_DIP`.
- `indicator_rect` — viewport-indicator overlay box. Proportional to the fraction of the buffer currently on screen.
...
When `total_lines * MINIMAP_LINE_HEIGHT_DIP > pane_h`, the minimap scrolls proportionally so the indicator stays aligned with the visible buffer region. When the full minimap fits, `scroll_y_dip = 0` and every line paints in place.
...
Left-click in the minimap strip centers the clicked source line in the
editor viewport; left-drag continues applying the same mapping until
button-up or capture loss. Hit-testing reads
...
## Tests
- `crates/render/src/minimap.rs::tests` — `short_buffer_does_not_stretch_minimap_lines` (the regression-fix invariant), `long_buffer_scrolls_minimap_proportionally`, `indicator_rect_tracks_visible_portion`, `strip_docks_at_right_edge_minus_inset`, `hit_test_*`.
