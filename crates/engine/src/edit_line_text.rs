//! Storage-neutral line-text edit planning — sort, reverse, unique, and trim
...
mod tests {
    ...
    #[test]
    fn indent_non_list_line_adds_line_prefix() {
        ...
        assert_eq!(b.rope().to_string(), "  plain");
        assert_eq!(b.selections()[0].head, Position::new(0, 5));
    }

    #[test]
    fn indent_multicursor_adds_one_prefix_per_covered_line() {
        let mut b = Buffer::from_text("alpha\nbeta");
        b.set_selections(vec![
            Selection::caret_at(Position::new(0, 2)),
            Selection::caret_at(Position::new(1, 3)),
        ]);
        run(
            &mut b,
            SelectionEdit::Indent {
        ...
        assert_eq!(b.rope().to_string(), "\talpha\n\tbeta");
        assert_eq!(
            b.selections()
                .iter()
                .map(|selection| selection.head)
                .collect::<Vec<_>>(),
            vec![Position::new(0, 3), Position::new(1, 4)]
        );
    }
